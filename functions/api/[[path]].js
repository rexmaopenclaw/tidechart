...
// ===== CACHE (in-memory for the Worker) =====
// Workers don't have persistent memory, but for this use case we can use the wrangler cache
// or KV. We'll use the built-in `caches` API for simplicity.
// Actually for 15-min TTL, we can use Cache API.
const CACHE_TTL = 15 * 60; // seconds

async function cachedFetch(url, options = {}, ttlSeconds = CACHE_TTL) {
  const cache = await caches.open('tide-app');
  ...
  if (resp.ok) {
    ...
    const cachedResp = new Response(respClone.body, { status: resp.status, headers });
...
function findNearestHKO(lat, lon) {
  ...
  for (const [code, stn] of Object.entries(HKO_STATIONS)) {
    ...
    if (d < bestDist) {
      bestDist = d;
      best = { code, name: stn.name, lat: stn.lat, lon: stn.lon, distance_km: Math.round(d * 111 * 100) / 100 };
    }
  }
  return best;
}
...
export async function onRequest(context) {
  ...
  try {
    ...
    // ---- Non-API routes: let Pages serve static files ----
    // This shouldn't normally be reached since Pages handles static files
